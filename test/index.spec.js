...
function transformFile(
    path,
    configuration = { config: './runtime.webpack.config.js' }
) {
    const out = transformFileSync(resolve(__dirname, path), {
        plugins: [['../../src/index.js', configuration]]
    })
...
test('basic require with the absolute resolve path', t => {
    const actual = transformFile('fixtures/basic.absolute.js', {
...
test('basic require with the relative resolve path', t => {
    const actual = transformFile('fixtures/basic.relative.js', {
        config: './runtime.webpack.config.js'
    })
    const expected = read('fixtures/basic.expected.js')
    t.is(actual, expected)
...
test('filename require', t => {
    const actual = transformFile('fixtures/filename.js', {
        config: './runtime.webpack.config.js'
    })
...
test('variable assignment', t => {
    const actual = transformFile('fixtures/variables.js', {
        config: './runtime.webpack.config.js'
    })
...
test('requiring files from the root', t => {
    const actual = transformFile('fixtures/rootfolder.js', {
        config: './runtime.webpack.config.js'
    })
...
test('using the import syntax', t => {
    const actual = transformFile('fixtures/import.js', {
        config: './runtime.webpack.config.js'
    })
...
test('aliasing a dependency', t => {
    const actual = transformFile('fixtures/dependency.js', {
        config: './runtime.webpack.config.js'
    })
    const expected = read('fixtures/dependency.expected.js')
